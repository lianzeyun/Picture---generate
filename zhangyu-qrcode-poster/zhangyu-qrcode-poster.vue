	...
	export default{
		...
		methods:{
			...
			async __init(){
				...
				//获取标题图片
				if(this.headerImg==''){
					var headerImg = await this.getImageInfo('@/static/defaultHead.png')
				}else{
					var headerImg = await this.getImageInfo(this.headerImg)
				}
				let hW = uni.upx2px(500);
				let hH = uni.upx2px(50);
				let hWs = uni.upx2px(100);
				let hHs = uni.upx2px(100);
				// //绘制标题图
				this.drawRoundImg(this.ctx,headerImg.path,((uni.upx2px(80)) / 2),((uni.upx2px(100)) / 2),hWs,hHs,26)
				//绘制标题
				this.ctx.setFontSize(20); //设置标题字体大小
				this.ctx.setFillStyle('#2E2E30'); //设置标题文本颜色
				this.ctx.fillText(this.title,((this.canvasW-hW) / 2),(
				(uni.upx2px(50) / 2) + hH + uni.upx2px(140)))
				//绘制职位
				this.ctx.setFontSize(10); //设置标题字体大小
				this.ctx.setFillStyle('#9B9B9B'); //设置标题文本颜色
				this.ctx.fillText(this.position,((this.canvasW-hW) / 2),(
				(uni.upx2px(60) / 2) + 20 + uni.upx2px(180)))
				//绘制公司
				this.ctx.setFontSize(12);
				this.ctx.setFillStyle('#2E2E30');
				let sWidth = this.ctx.measureText(this.subTitle).width
				if(sWidth > hWs){
					this.ctx.fillText(this.subTitle.slice(0,13) + '...',((this.canvasW-hW) / 2),(
					(uni.upx2px(300) / 2) + hH + uni.upx2px(110)))
				}else{
				...
				this.ctx.fillText(phone + ' / ' + mailbox,((this.canvasW-hW) / 2),((uni.upx2px(320) / 2) + 20 + uni.upx2px(150)))
				//绘制价格
				...
				// this.ctx.fillText('长按识别图中“馋么”小程序',(((this.canvasW-hW) / 2) + uni.upx2px(34)),(((this.canvasW-hW) / 2) + hH + uni.upx2px(300)))
				//底部广告
				...
				this.drawRoundImg(this.ctx,abImg.path,((uni.upx2px(950)) / 2),((uni.upx2px(470)) / 2),hWs,hHs,26)
				//延迟渲染
				setTimeout(()=>{
					this.ctx.draw(true,()=>{
						uni.hideLoading()
